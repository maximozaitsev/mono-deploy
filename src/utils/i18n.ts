// src/utils/i18n.ts
// Deprecated legacy i18n utilities kept temporarily for backward compatibility during migration
// New code should use next-intl. These exports are no-ops or minimal shims.
...
const __ST = {} as unknown as RawStaticMap;
...
export function useStaticT() {
  ...
  const value = useMemo(() => {
    return { t: {} as any, currentLang: getLangFromPath(pathname || "/", __manifest), manifest: __manifest };
  }, [pathname]);
  return value;
}
